...
# Function to fix rupee symbol issue
def fix_currency_symbols(text: str) -> str:
    # Replace '2' with '₹' rupee symbol (based on OCR misinterpretation)
    text = text.replace('2', '₹')
    return text
...
def run_tesseract(state: ReimburesemtAgent):
    ...
    # Fix the rupee symbol issue
    fixed_text = fix_currency_symbols(text)

    new_uuid = uuid.uuid4()
    return {'extracted_text': fixed_text, 'unique_id': new_uuid}
...
# Gradio Interface
logo_path = "/content/reimburse-ocr/images/elint_logo.jpeg"
contact_info = """
### Elint Data Private Limited
**Office No 4, First Floor, Street No 3, New Delhi, Central Delhi**
**GSTIN/UIN**: 07AAGCE9477N1ZS
**State Name**: Delhi, Code: 07
**Contact**: 9811290063
"""

# Define a Gradio interface with the logo and contact info
def show_logo_and_contact():
    return gr.Image(logo_path), gr.Markdown(contact_info)
...
# Adding the logo and contact info using `show_logo_and_contact` method
with gr.Blocks() as demo:
    show_logo_and_contact()
    iface.render()
